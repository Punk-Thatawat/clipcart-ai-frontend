...
export function AiGeneratorDrawer({ product, open, onClose }: { product: Product | null; open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-[#11162e]/35 backdrop-blur-[2px] animate-fade-in" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <aside className="h-full w-full max-w-[860px] overflow-y-auto bg-[#fffaf6] shadow-2xl animate-slide-in">
        ...
        <div className="space-y-6 p-5 md:p-7">
          ...
          {generation.status === "complete" && generation.result && <GeneratedResult result={generation.result} onRegenerate={generation.generate} showStoryboard={generation.settings.includeStoryboard} />}
        </div>
      </aside>
    </div>
  );
