...
import { AiVoiceSettings } from "./ai-voice-settings";
...
function SettingsSelect({ label, value, options, onSelect }: { label: string; value: string; options: string[]; onSelect: (value: string) => void }) {
  return (
    <label className="block">
      ...
      <select value={value} onChange={(e) => onSelect(e.target.value)} className="focus-ring h-11 w-full rounded-xl border border-[#e8e1dc] bg-white py-0 pl-4 pr-10 text-sm">
...
export function GenerationSettings({ settings, onChange }: { settings: Settings; onChange: (settings: Settings) => void }) {
  ...
  return (
    <div className="grid gap-y-4 sm:grid-cols-2 sm:gap-x-6">
      ...
      <AiVoiceSettings settings={settings} onChange={onChange} />
      <div className="space-y-2 sm:col-span-2">
        <label className="flex items-center gap-3 rounded-xl bg-[#fff5f0] p-4 text-sm font-medium"><input type="checkbox" checked={settings.includeCta} onChange={(e) => update("includeCta", e.target.checked)} className="size-4 accent-[#ff5a36]" /> Include affiliate call-to-action</label>
        <label className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition ${settings.includeStoryboard ? "border-[#ffc5b5] bg-[#fff7f3]" : "border-[#e8e1dc] bg-white hover:border-[#f2c8bb]"}`}>
          <input
            type="checkbox"
            checked={settings.includeStoryboard}
            onChange={(e) => update("includeStoryboard", e.target.checked)}
            className="mt-0.5 size-4 accent-[#ff5a36]"
          />
          <span>
            <span className="block text-sm font-semibold">Include detailed storyboard</span>
            <span className="mt-1 block text-xs leading-5 text-[#7a7f8e]">
              Optional shot-by-shot plan for creators who need production guidance.
            </span>
          </span>
        </label>
      </div>
